...
export const actions = {
  async fetchAllEpisodes ({ commit }, payload) {
    try {
      const { data } = await episodeService().getAllEpisodes(payload.productId, payload.page, payload.size)
      commit('SET_EPISODES', data.data.items)
      return data
    } catch (e) {
      console.log(e)
    }
